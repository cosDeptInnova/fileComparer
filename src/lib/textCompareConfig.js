...
export const DEFAULT_TEXT_COMPARE_CAPABILITIES = {
  ...
  allowed_extensions_label: ".pdf, .docx, .txt, .xlsx",
  conditional_extensions: {},
  engines: {
    default: "auto",
    available: ["auto", "builtin"],
    declared: ["auto", "builtin", "docling"],
    availability: { auto: true, builtin: true, docling: false },
  },
  soffice: { available: false, path: null },
  ocr: { available: false, backend: null },
  max_file_mb: 40,
...
export function normalizeTextCompareCapabilities(payload = {}) {
  ...
  return {
    ...
    conditional_extensions: payload.conditional_extensions || DEFAULT_TEXT_COMPARE_CAPABILITIES.conditional_extensions,
    engines: payload.engines || DEFAULT_TEXT_COMPARE_CAPABILITIES.engines,
    soffice: payload.soffice || DEFAULT_TEXT_COMPARE_CAPABILITIES.soffice,
    ocr: payload.ocr || DEFAULT_TEXT_COMPARE_CAPABILITIES.ocr,
    messages: {
