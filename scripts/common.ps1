...
function Test-LogContainsAnyPattern {
  param(
  ...
  if ($candidatePatterns.Count -eq 0) { return $true }

  foreach ($path in @($Paths | Where-Object { $_ })) {
    if (-not (Test-Path $path)) { continue }
    foreach ($pattern in $candidatePatterns) {
      if (Test-FileContainsText -Path $path -Pattern $pattern) {
        return $true
      }
    }
  }
...
function Wait-NonPortServiceReady {
  ...
  $readyLogPatterns = @(As-Array (Try-GetProp -Obj $Svc -Name "ReadyLogPattern" -Default $null))
  ...
  if (-not $requireAllInstances -and ($readyLogPatterns.Count -eq 0)) {
  ...
  while ($sw.Elapsed.TotalSeconds -lt $readyTimeoutSec) {
    ...
    foreach ($instanceNumber in $targetInstances) {
      ...
      if ($readyLogPatterns.Count -gt 0) {
        $logs = Get-LogFiles -Root $Root -ServiceName $name -InstanceNumber $instanceNumber
        if (-not (Test-LogContainsAnyPattern -Paths @($logs.Out, $logs.Err) -Patterns $readyLogPatterns)) {
          $allReady = $false
        }
      }
    }
    ...
    $readyOk = (($readyLogPatterns.Count -eq 0) -or $allReady)
  ...
  foreach ($instanceNumber in $targetInstances) {
    ...
    $pidFile = Get-PidFile -Root $Root -ServiceName $name -InstanceNumber $instanceNumber
    $logs = Get-LogFiles -Root $Root -ServiceName $name -InstanceNumber $instanceNumber
    $wrapperPid = Read-PidFileSafe -PidFile $pidFile
    $alive = if ($wrapperPid) { Test-ProcessAlive -ProcessId $wrapperPid } else { $false }
    $patternSeen = if ($readyLogPatterns.Count -gt 0) { Test-LogContainsAnyPattern -Paths @($logs.Out, $logs.Err) -Patterns $readyLogPatterns } else { $true }
    $tail = Get-LogTailText -Path $logs.Err -Lines 20
    if (-not $tail) { $tail = Get-LogTailText -Path $logs.Out -Lines 20 }

    $details.Add(("{0}: wrapper_pid={1}; alive={2}; ready_log={3}; err_tail={4}" -f $instanceLabel, $wrapperPid, $alive, $patternSeen, $tail))
  }
